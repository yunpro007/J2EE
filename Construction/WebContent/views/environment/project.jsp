...
<body>
	<nav id="content-header"> <B>位置：</B>环境监测</nav>
	<div class="header-query">
		<div class="row" style="margin: 0px;">
			<div class="col-md-12">
				<form method="post"
					action="<%=path%>/environment?method=queryByCondition">
					<div class="row">
						<div class="col-sm-4">
							<label class="form_query">工程名称&nbsp;:&nbsp;</label><input type="text"
								name="qoprojectName"  id="qoprojectName" />
						</div>
						<div class="col-sm-4">
							<label class="form_query">工程地址&nbsp;:&nbsp;</label><input type="text"
								name="qolocation" id="qolocation" />
						</div>
					...
					<div class="row">
						<div class="col-sm-4">
							<label class="form_query">建设单位&nbsp;:&nbsp;</label><input type="text"
								name="qoinvestor" id="qoinvestor" />
						</div>
						<div class="col-sm-4">
							<label class="form_query">施工单位&nbsp;:&nbsp;</label><input type="text"
								name="qobuilder" id="qobuilder" />
						</div>
						<div class="col-sm-4">
							<button type="submit" class="query" onclick="query()">
						...
						<c:forEach var="ins" items="${results}" varStatus="vs">
							<tr class="bggray"
								...
								<td align="center" valign="middle" width="100"
									...
									href="javascript:location.href=encodeURI('<%=path%>/environment?method=showEnvirDetection&aq=${ins.aq1}&pno=${ins.pno}&projectName=${ins.projectName}')">${ins.projectName}</a>
								</td>
								<td align="center" valign="middle" width="100"
...
<%--    function  query(){
	   jQuery.ajax({
			...
			data:{
				...
				qoprojectName:$("#qoprojectName").val(),
				qolocation:$("#qolocation").val(),
				qoinvestor:$("#qoinvestor").val(),
				qobuilder:$("#qobuilder").val()
			} ,
			success:function(data){
				var  array =  eval('(' + data + ')');
				showData(arrat);
				/* alert(array.sum);
			    alert(array["results"][0]["pno"]); */
			},
   ...
   function  showData(data){
	   var sum = data.sum ;
	   var div = document.getElementById("main-tab");
